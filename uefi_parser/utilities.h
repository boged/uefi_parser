...
public:
    ...
    template <typename StreamType>
    void print(StreamType& stream, std::string tableName)
    {
        ...
        // Print out the top line
        stream << std::string(total_width, '_') << "\n";
        stream << '|';
        stream << std::setw(total_width - 2) << centered(tableName);
        //stream 
        stream << '|' << std::endl;
        stream << std::string(total_width, '-') << "\n";
        // Print out the headers
...
//};
//
//void print_format_string(std::vector<ROW_FIELD> rowFields, uint8_t mode);
